...
cqreglwr <- function(form,taumat=c(.10,.25,.50,.75,.90),window=.25,bandwidth=0,kern="tcub",distance="Mahal",target=NULL,data=NULL) {
  ...
  if (identical(target,NULL)){
    target <- McSpatial::maketarget(form,window=window,bandwidth=bandwidth,kern="tcub",data=data)$target
  }
  alldata = FALSE
  ...
  for (i in seq(1:nt)) {
    ...
    # hacking replace
    mod <- rq.fit.hogg(as.matrix(xmat[samp,1] * k), y[samp] * k, weights=rep(1, ntau), taus=taumat)
    print(xmat[samp,1])
    print(y[samp])
    print(xmat[samp,1] * k)
    print(y[samp]*k)
    ...
    if (nk==1) {fit <- rq.fit.hogg(as.matrix(x1 * k), y[samp] * k, weights=rep(1, ntau), taus=taumat)}
    if (nk==2) {stop("not support nk==2 yet")}
